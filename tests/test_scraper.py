...
class SidecarXmlFallbackTests(unittest.TestCase):
    def setUp(self) -> None:
        self.scraper = SwimMeetScraper()
    ...
    @mock.patch("swimmeet_scraper.scraper.urlopen")
    def test_fetches_sidecar_xml_when_html_lacks_results(self, mock_urlopen: mock.Mock):
        mock_urlopen.side_effect = [
            self._response("<html><xml><results></results></xml></html>", "text/html"),
            self._response(
                "<xml><results><result rk='1' nm='Sidecar Swimmer' sc='XYZ' ti='50.50' mt='meet' auto='yes'/></results></xml>",
                "text/xml",
            ),
        ]
